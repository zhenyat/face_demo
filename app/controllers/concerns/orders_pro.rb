...
module OrdersPro
  ...
  def cancel_order order
    if order.x_id.nil?
      ...
      response  = Face.cancel_order order_id: order.x_id
  ...
  def check_fix_order order
    response = Face.order_info order.x_id
  ...
  def place_order order, fix = false
    ...
    if fix
      ...
      type = run.kind                               # The same as Run's kind
    end
    
    pair_name = run.pair.name
    response  = Face.trade pair: pair_name, type: type, rate: order.rate.round(3), amount: order.amount
...
#    rates = Array.new(run.orders_number)
  ...
  def trace_order order
    response = Face.order_info order_id: order.x_id
