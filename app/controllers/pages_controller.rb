...
#    pairs = 'btc_usd'
    ...
    depth = Face.depth pairs: pairs, limit: limit
  ...
  def public_api
    pairs = %w(btc_usd eth_usd ltc_usd).join('-')
    @info   = Face.info
    @ticker = Face.ticker pairs: pairs
    @depth  = Face.depth  pairs: pairs, limit: 20
    @trades = Face.trades pairs: pairs, limit: 30
    puts @trades
  end

  def trade_api
    if session['HTTP_REFERER'].nil?
      puts "Debug! session['HTTP_REFERER'] =  nil"
    else
      puts "Debug! #{session['HTTP_REFERER']}"
    end
    @account_info  = Face.account_info
    @active_orders = Face.active_orders 'mode' => 'emulator'
    @order_info    = Face.order_info 343152, 'mode' => 'emulator'
  end
...
#    pairs = 'btc_usd'
    ...
    @trades = Face.trades pairs: pairs, limit: limit
